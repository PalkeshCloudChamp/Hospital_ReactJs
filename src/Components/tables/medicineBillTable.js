...
class MedicineBillTable extends Component {
    constructor(props) {
        super(props);
        this.state = {
            data : [],
    ...
    componentDidMount(prevProp){
        this.secDoc.getData("http://localhost:9080/api/showMedicineBill").then(resp=>{
        ...
        if(tokenDesi== null || tokenVal == null)
    {
        ...
        {this.state.data.length>0 ? <TableDom showAdd = {this.state.showAdd} showUpdate = {false} showDelete = {false} headers={Object.keys(this.state.data[0])} body= {this.state.data} getLink = "http://localhost:9080/api/showMedicineBill" addURL = "/addMedicineBill" props={this.props} pkId = {Object.keys(this.state.data[0])[0]}/> : null}
        </div></div></> );
    }
}

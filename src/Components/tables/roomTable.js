...
class RoomTable extends Component {
    ...
    componentDidMount(prevProp){
        this.secDoc.getData("http://localhost:9080/api/showRoom").then(resp=>{
        ...
        if(tokenDesi== null || tokenVal == null)
    {
        ...
        {this.state.data.length>0 ? <TableDom showAdd = {this.state.showAdd} showDelete = {this.state.showAdd} showUpdate = {this.state.showAdd} headers={Object.keys(this.state.data[0])} body= {this.state.data} link="http://localhost:9080/api/deleteRoom" getLink = "http://localhost:9080/api/showRoom" updateURL = "/updateRoom" addURL = "/addRoom" props={this.props} pkId = {Object.keys(this.state.data[0])[0]}/> : null}
        </div></div></> );
    }
}
